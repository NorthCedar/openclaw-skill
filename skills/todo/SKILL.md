...
## Calendar Sync Rule
When a captured item has a specific date/time (deadline, appointment, or scheduled action):
1. Add it to todo as normal
2. ALSO create a calendar event via khal:
   ```bash
   khal new <date> <start> <end> "<title>" -a home
   vdirsyncer sync
   ```
   - If no specific time, create as all-day event
   - Default calendar: `home` (use `work` if clearly work-related)
